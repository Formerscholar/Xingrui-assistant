<template>
  <div class="page">
    <el-dialog title="导入" v-model="dialogVisible" width="70rem" center @close="onDialogClose">
      <el-row :gutter="20">
        <el-col :span="24">
          <div class="title">
            <h4>导入数据预览<el-link type="primary" style="margin-left: 20px" :href="template_rul">模板下载</el-link></h4>
            <el-upload
                :action="upload_url"
          ...
          >
            ...
            <el-table-column prop="team" label="班级"></el-table-column>
            <el-table-column prop="teacher" label="负责的老师"></el-table-column>
            <el-table-column prop="mobile" label="联系方式"></el-table-column>
            <el-table-column prop="saler" label="销售人员"></el-table-column>
...
import {studentImport} from '@/network/student'
...
export default {
  ...
  setup(props, context) {
    ...
    const state = reactive({
      dialogVisible:false,
      list:[],
      upload_url:"/ins/Student/upload",
      template_rul:"/ins/block/downTpl?type=student",
    })
    ...
    const handleImport = async () => {
      let {code,data} = await studentImport({
        list:state.list
      })
      if(!code)
      {
